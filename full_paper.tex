...
Email: alex.prokopyszyn@tokamakenergy.com
...
We model the backgound quantities as fixed. Fig. \ref{fig:background_plasma_profiles} shows profiles of the background plasma as a function of the normalised poloidal flux $\psi_N$. 
...
\end{equation}
where $\text{E}(E_j)=3.5\,\text{MeV}$ is the mean birth energy of the $\alpha$-particles from the nuclear fusion, $m_\alpha$, is the mass of an $\alpha$-particle, $m_n$ is the mass of a neutron, $T_i$ is the background temperature of the ions in eV. In the core, the standard deviation of the energy is about 10\%. {\bf Note that we do not use a slowing-down distribution for the $\alpha$-particles, as this section focuses on calculating the fraction of fusion $\alpha$-particle power at steady-state that reaches the PFCs. In our simulation, the $\alpha$-particle population is not continuously replenished, which is a necessary condition for sustaining a slowing-down distribution.}
...
There are sixteen of each type of ELM suppression coil in each row, as illustrated in Fig. \ref{fig:coil_plot_3d}. The current in the $k^\text{th}$ coil of the upper and lower rows is given by
\begin{align}
...
\begin{figure}[!ht]
    ...
    % \caption{Caption}
    % \caption{\parbox{0.9\linewidth}{This figure shows the results from 96 simulations. The left-columns s on the wall in $\text{MW\,m}^{-2}$}}
    \caption{As Fig. \ref{fig:max_and_total_flux_vs_phase_elm_rwm} except that coils exterior to the vacuum vessel are employed to suppress ELMs in this case.}
...
%     \caption{Distribution of peak $\alpha$-particle energy flux maximised over toroidal angle in poloidal cross-section (left) and versus poloidal distance $s_\theta$ along the wall (right plot). Both graphs display the maximum alpha particle energy flux, denoted as $S_{max}$, measured in $\text{MW\,m}^{-2}$. The definition of $S_{max}$ can be found in Equation \eqref{eq:max_alpha_particle_energy_flux}. \textbf{I think split this into two rows so we can make both figures signficantly bigger. We need to show DPLOTs of the orbits. Also show this for the full 3D field with RWM and ripple as well.}}
...
Sensors are located near the active control coils (see Fig. \ref{fig:coil_plot_3d}), measuring the field intensity and initiating the feedback cycle in response.
...
Figure \ref{fig:energy_flux_full_3d} highlights two major hot spots. The hot spot in the upper dome is labeled as `prompt losses' because most of the markers that impact this region of the wall do so within \(10^{-5}\,\text{s}\) after being generated through nuclear fusion. These markers are generated by $\alpha$-particles born on orbits that quickly take them out of the plasma. The hot spot in the outer leg of the bottom divertor is labelled as `collisional losses' because the $\alpha$-particles take significantly longer to reach this area after their generation. {\bf Initially, the particles are well-confined, but they begin to drift due to collisions, a process that is further accelerated by the three-dimensional field. The `collisional' hot spot does not appear in axisymmetric simulations, and its amplitude is significantly diminished in the absence of collisions.}.

{\bf Finally, we note that the losses observed in the full 3D field scenario are nearly identical (within Monte Carlo noise) to those obtained using only the RMP field under the same conditions. This suggests that, in this case, the additional 3D field components (TF ripple and RWM) have a negligible impact on alpha particle confinement. A more systematic investigation into the interplay between different 3D field components and their combined effects on confinement would be a valuable direction for future research.}

\begin{figure}
    \centering
    \includegraphics[width=0.99\linewidth]{Figures/energy_flux_full_3d.pdf}
    \caption{Distribution of peak $\alpha$-particle energy flux maximised over toroidal angle in the R-Z plane (top) and versus poloidal distance, $s_\theta$, along the wall (bottom). Both graphs display the maximum alpha particle energy flux, denoted as $S_{max}$, measured in $\text{MW\,m}^{-2}$. The definition of $S_{max}$ can be found in Equation \eqref{eq:max_alpha_particle_energy_flux}}
...
Future research in the field of Alfv\'en eigenmode stability in STEP will focus {\it inter alia} on the potential destabilisation of these modes by fast electrons during the ramp-up phase (when electron cyclotron current drive will be employed \cite{Henderson2024}) and by runaway electrons during disruptions. Additionally, the impact of the $q$-profile on Alfv\'en eigenmode stability during the flat-top phase will be explored. We plan to investigate the stability of ellipticity-induced Alfv\'en eigenmodes and noncircular triangularity-induced Alfv\'en eigenmodes \cite{betti1992} as well as higher frequency compressional and global Alfv\'en eigenmodes \cite{Gorelenkov2016}. Modes with finite compressibility can also exist at frequencies below the TAE range, and can couple to shear waves (see for example \cite{Gorelenkov2007}). {\bf Moreover high plasma elongation can give rise to gaps in the coupled shear and acoustic continua, within which additional global eigenmodes can exist \cite{Rodrigues2021}}. In view of the high beta regime of flat-top STEP plasmas {\bf and their high elongation, these modes} should be included in future studies. 

{\bf We end with a brief comparison of our results with those reached in predictive studies of $\alpha$-particle physics in other tokamak burning plasma concepts. The investigation of fast ion losses from SPARC in Ref. \cite{Scott2020} concluded that TF ripple-induced losses of fusion $\alpha$-particles would be acceptably low in the case of perfectly-aligned TF coils. Setting the maximum ripple amplitude equal to the value in SPARC (0.15-0.30\%), we find from Fig. 3 that ripple-induced losses in STEP would be at axisymmetric levels, which are also acceptably low. Fast ion losses due to RMP coils in ITER have been modelled in a number of papers, for example Refs. \cite{ward2022,Varje2016}. In contrast to Figs. \ref{fig:max_and_total_flux_vs_phase_elm_rwm} and \ref{fig:max_and_total_flux_vs_phase_elm}, Varje and co-workers \cite{Varje2016} found that the plasma response reduces $\alpha$-particle losses (although it increases neutral beam fast ion losses). The most significant difference in terms of fast particle physics between STEP and other tokamak burning plasma concepts is in terms of the role likely to be played by Alfv\'enic instabilities. There is a significant chance that these will have an impact on $\alpha$-particle and possibly core plasma transport in conventional tokamak burning plasmas, including those in ITER \cite{Pinches2015}, but a robust conclusion of the present work is that that they will not be excited (at least by fusion $\alpha$-particles) in STEP.}    
...
This work has been funded by STEP, a major technology and infrastructure programme led by UK Industrial Fusion Solutions Ltd (UKIFS), which aims to deliver the UK’s prototype fusion powerplant and a path to the commercial viability of fusion. The authors thank Antti Snicker and Konsta S\"arkim\"aki at VTT Technical Research Centre of Finland for their help in verifying our results using the ASCOT \cite{hirvijoki2014} code.
...
\bibitem{Gorelenkov2007} % journal article
% example: AUTHOR, A., AUTHOR, B., AUTHOR, C., Journal article title in sentence case, Abb. J. Title 1 2 (Year) 120–123.
...
\bibitem{Pinches2015} % journal article
% example: AUTHOR, A., AUTHOR, B., AUTHOR, C., Journal article title in sentence case, Abb. J. Title 1 2 (Year) 120–123.
...
\bibitem{Rodrigues2021} % journal article
% example: AUTHOR, A., AUTHOR, B., AUTHOR, C., Journal article title in sentence case, Abb. J. Title 1 2 (Year) 120–123.
    RODRIGUES, P., CELLA, F., 
    High-order geodesic coupling of shear-Alfv\'en and acoustic continua in tokamaks,
    Nucl. Fusion,
    \textbf{61}
    (2021) 
    096001.
...
\bibitem{Varje2016} % journal article
% example: AUTHOR, A., AUTHOR, B., AUTHOR, C., Journal article title in sentence case, Abb. J. Title 1 2 (Year) 120–123.
    VARJE, J., ASUNTA, O. CAVINATO, M., GAGLIARDI, M., HIRVIJOKI, E., KOSKELA, T, KURKI-SUONIO, T., LIU, Y.Q., PARAIL, V., SAIBENA, G., SIPIL\"A, S., SNICKER, A., S\"ARKIM\"AKI, K., \"AK\"ASLOMPOLO, S., 
