...
The forces on the marker due to collision**s with the background ions and electrons are $\textbf{F}_{\alpha,i}(\mathbf{x}_j)$ and $\textbf{F}_{\alpha,e}(\mathbf{x}_j)$ respectively, while the mass and charge of the $\alpha$-particles are $m_\alpha$ and $q_\alpha$ respectively. The equation of motion for the $j^{\text{th}}$ marker is
\begin{equation}
...
\begin{figure}[!htb]
    ...
    \caption{Schematic representation of STEP’s last closed flux surface, ELM suppression coils, Resistive Wall Mode (RWM) active control coils and TF coils. These designs may change in the future. 
}
...
\begin{figure}[!htb]
    \centering
    \includegraphics[width=0.7\linewidth]{Figures/energy_flux_distribution_rmp.pdf}
